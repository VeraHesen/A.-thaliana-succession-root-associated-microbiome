...
# Input: 
## CSS_normalized_ASV_16S_VH
## extradata_16S_VH
## raw_count_data_16S_VH
...
df <- data.frame(read.csv("CSS_normalized_ASV_16S_VH.csv", header = TRUE, stringsAsFactors=F, row.names = 1))
...
df_t <- t(df)
NMDS <- metaMDS(df_t, distance = "bray", k = 3) 
...
capture.output(permanova, file = "NMDS.16S_permanova.txt")
...
write_xlsx(permanova.pair, "NMDS.16S_permanova_pairwise.xlsx")
...
ggplot(scores, aes(x = NMDS1, y = NMDS2, colour = factor(Succession, levels = succ_order), 
  ...
  annotate("text", x = -1.5, y = 0.55, label = "succession (F = 3.3146 and P = 0.001***)", hjust = 0, size = 3) +
  annotate("text", x = -1.5, y = 0.45, label = "succession:site (F = 2.4386 and P = 0.001***)", hjust = 0, size = 3) +
  scale_shape_manual(values = c(19, 17, 15, 8)) 
  # position of annotate text is set manually, depends on data
ggsave2(filename= "NMDS.16S_succession.png", device = "png", width=8, height=4)

# plot with field and succession info
ggplot(scores, aes(x = NMDS1, y = NMDS2, colour = factor(Succession, levels = succ_order),
                                                         shape = factor(Succession, levels = succ_order))) +
  geom_segment(data = seg_f,mapping = aes(xend = oNMDS1, yend = oNMDS2)) + # add spiders
  geom_point() + # add all data points
  coord_fixed()+                                              
  theme_bw()+
  theme(legend.position="right",legend.text=element_text(size=10),
        legend.direction='vertical', legend.title = element_blank()) +
  annotate("text", x = -1.6, y = 0.65, label = "succession (F = 3.3146 and P = 0.001***)", hjust = 0, size = 3) +
  annotate("text", x = -1.6, y = 0.55, label = "succession:site (F = 2.4386 and P = 0.001***)", hjust = 0, size = 3) +
  scale_shape_manual(values = c(19, 17, 15, 8)) +
  geom_point(data = centroids_f, size = 4)
  # position of annotate text is set manually, depends on data
ggsave2(filename= "NMDS.16S_fields_succ3.png", device = "png", width=8, height=6)
...
write_xlsx(df_extra_ma10, "16S_10ma.xlsx")
df_extra_ma10
...
capture.output(print(paste(perc_ma10,"% of total normalized reads are normalized reads of the 10 most abundant 16S OTUs")), file = "16S_10ma_perc.txt")
...
total_10ma$OTU <- factor(total_10ma$OTU, 
                             levels = c("OTU_16S_1","OTU_16S_2","OTU_16S_3","OTU_16S_4","OTU_16S_5",
                                        "OTU_16S_6","OTU_16S_7","OTU_16S_8","OTU_16S_9","OTU_16S_10"))
...
OTU1 <- paste(df_extra_ma10$Genus[[1]],"(G.)")
OTU2 <- paste(df_extra_ma10$Genus[[2]],"(G.)")
OTU3 <- paste(df_extra_ma10$Family[[3]],"(F.)")
OTU4 <- paste(df_extra_ma10$Genus[[4]],"(G.)")
OTU5 <- paste(df_extra_ma10$Order[[5]],"(O.)")
OTU6 <- paste(df_extra_ma10$Genus[[6]],"(G.)")
OTU7 <- paste(df_extra_ma10$Genus[[7]],"(G.)")
OTU8 <- paste(df_extra_ma10$Family[[8]],"(F.)")
OTU9 <- paste(df_extra_ma10$Family[[9]],"(F.)")
OTU10 <- paste(df_extra_ma10$Genus[[10]],"(G.)")
...
ggplot(total_10ma, aes(x = succession, y = OTU, fill = perc_reads)) + 
        geom_tile() +
        scale_y_discrete(limits=rev, labels = rev(c(expression(italic("Bradyrhizobium (G.)")),
                                                    expression(italic("Pseudarthrobacter (G.)")),
                                                    expression(italic("Xanthobacteraceae (F.)")),
                                                    expression(italic("Sphingomonas (G.)" )),
                                                    expression(italic("Bacillales (O.)" )),
                                                    expression(italic("Acidothermus (G.)" )),
                                                    expression(italic("Hyphomicrobium (G.)")),
                                                    expression(italic("Xanthobacteraceae (F.)")),
                                                    expression(italic("Planococcaceae (F.)")),
                                                    expression(italic("Piscinibacter (G.)"))))) +
...
ggsave2(filename= "16S_10ma_heatmap.png", device = "png", width=4, height=4)      
...
venn1
ggsave2(filename= "venn1_16S.png", device = "png", width=6, height=4)
